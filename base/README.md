...
`docker-compose.yml` builds the image, mounts `setup.json` and the `entrypoint/`
files directory at the expected paths, loads `.env`, and starts a `docker:dind`
sidecar (`docker-daemon`) that the container's Docker CLI talks to via
`DOCKER_HOST=tcp://docker-daemon:2375` — no host Docker socket required:
...
Swap or drop the `docker-daemon` service (and the `DOCKER_HOST` override) if
you'd rather mount the host's Docker socket instead.
...
```bash
python3 --version && pip3 --version && python3.14 --version && pip3.14 --version
